...
export class SystemGroupController {
  ...
  updateById = async (req: Request, res: Response): Promise<void> => {
    try {
      const data = await this.service.updateById(req.params.id as string, req.body);
      res.status(200).json({ success: true, data });
    } catch (error) {
      const httpError = toSafeHttpError(error);
      const message = httpError.expose ? httpError.message : "Internal server error";
  ...
  updateMemberById = async (req: Request, res: Response): Promise<void> => {
    try {
      const data = await this.service.updateMemberById(
  ...
  deleteMemberById = async (req: Request, res: Response): Promise<void> => {
    try {
      await this.service.deleteMemberById(req.params.groupId as string, req.params.memberId as string);
      res.status(204).send();
    } catch (error) {
      const httpError = toSafeHttpError(error);
      const message = httpError.expose ? httpError.message : "Internal server error";
