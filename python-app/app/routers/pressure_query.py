...
from app.schemas.pressure_query import PressureQueryResponse
...
@router.get(
    "/pressureBySystem",
    response_model=PressureQueryResponse,
    summary="Tags de presión (PRESSURE / PRESSURE_IN / PRESSURE_OUT) de un sistema, resampleados por intervalo",
)
async def pressure_by_system(
    ...
    cache_key = f"pressure:{system_code}:{start.isoformat()}:{end.isoformat()}:{interval_seconds}"
    cached = await cache.get(cache_key)
    if cached:
        return PressureQueryResponse.model_validate_json(cached)

    try:
        tags = await get_pressure_by_system(system_code, start, end, interval_seconds)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"Error consultando backend: {exc}")

    response = PressureQueryResponse(
